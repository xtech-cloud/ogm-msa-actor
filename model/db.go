...
func AutoMigrateDatabase() {
	err := DefaultConn.DB.AutoMigrate(&Domain{})
	if nil != err {
		logger.Fatal(err)
	}
	err = DefaultConn.DB.AutoMigrate(&Device{})
	if nil != err {
		logger.Fatal(err)
	}
	err = DefaultConn.DB.AutoMigrate(&Guard{})
	if nil != err {
		logger.Fatal(err)
	}
	err = DefaultConn.DB.AutoMigrate(&Application{})
	if nil != err {
		logger.Fatal(err)
	}
}
