...
call:
	...
	gomu --registry=etcd --client=grpc call xtc.ogm.actor Domain.AcceptDevice '{"uuid":"${UUID}"}'
	gomu --registry=etcd --client=grpc call xtc.ogm.actor Domain.RejectDevice '{"uuid":"${UUID}"}'
	gomu --registry=etcd --client=grpc call xtc.ogm.actor Sync.Push '{"domain":"${UUID}", "device":{"serialNumber":"00001", "name":"G2", "operatingSystem":"Windows 11", "systemVersion":"21H2", "shape":"PC", "program":{"myapp":"1.0.0"}}, "upProperty":{"key1":"value1", "key2":"value2"}, "downProperty":["key1"]}'
